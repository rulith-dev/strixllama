...
| **Drafts sized by concurrency** | In this mixture of experts every verified draft token reads the weights of ~10 more experts, which conversations decoding together cannot share: four at once verified 16 tokens a step. The draft now shrinks as more conversations generate (3, then 2, none from four on): four conversations 37.9 → 47.1 tok/s summed. |

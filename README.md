...
The whole delta against upstream llama.cpp is **24 files** — 20 modified, 4 added, out of 3610. The
substantial pieces:
...
```bash
python tools/replay_bootstrap.py          # clean upstream + patch set == the 24 files, byte for byte
```

It restores the 20 modified files to upstream from the clone's own git objects, addressed by the blob
hashes in `bootstrap/UPSTREAM.json` — so clean upstream is reconstructed rather than trusted — then
replays the snapshot and every script and compares. It reports **24 / 24**.

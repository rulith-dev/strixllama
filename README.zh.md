...
本机用 0.2.3 实测（2026-09-26；多路用 0.2.4），推测解码开启：预填充为 95.6K token 真实文本；解码为在同一文本的
86K token 之后、以及只有一句提问之后各生成 400 token；多路为几个各约 4K 上下文的对话同时解码，每轮换一个采样种子：
...
| 解码，3 / 4 路同时 | **53.6 / 61.4 tok/s** 合计（同样测法下单路的 1.50 / 1.71 倍） |
...
相对上游 llama.cpp 的全部改动是 **53 个文件**——3610 个里改了 49 个、新增 4 个。主要几项：
...
```bash
python tools/replay_bootstrap.py          # 干净上游 + 补丁集 == 那 53 个文件，逐字节相同
```

它按 `bootstrap/UPSTREAM.json` 里记的 blob 哈希，从克隆自带的 git 对象里取回那 47 个文件的上游版
本——干净上游是重建出来的，不是信来的——然后重放整套快照和脚本再比对。结果是 **53 / 53**。

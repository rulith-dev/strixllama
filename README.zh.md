...
| **按并发调整草稿长度** | 这个 MoE 里每个待验证的草稿 token 都要多读约 10 个专家的权重，同时解码的几个对话无法共享：四路同时时每步要验证 16 个 token。现在同时生成的对话越多，草稿越短（3，然后 2，四路起不起草）：四路合计 37.9 → 47.1 tok/s。同一步里各路草稿也补齐到相同长度：长短不一的草稿会让混合记忆把一次验证拆成整个模型的好几次前向，三路 4K 上下文开草稿只有 34 tok/s、不开反而 46；现在开草稿也是 46。 |

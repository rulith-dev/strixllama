...
相对上游 llama.cpp 的全部改动是 **29 个文件**——3610 个里改了 25 个、新增 4 个。主要几项：
...
```bash
python tools/replay_bootstrap.py          # 干净上游 + 补丁集 == 那 29 个文件，逐字节相同
```

它按 `bootstrap/UPSTREAM.json` 里记的 blob 哈希，从克隆自带的 git 对象里取回那 25 个文件的上游版
本——干净上游是重建出来的，不是信来的——然后重放整套快照和脚本再比对。结果是 **29 / 29**。

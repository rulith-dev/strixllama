...
量化也能加载，但没有针对它们调过、也没验证过。草稿和投影**默认都是开的**，开着而文件不在时加载会直接
拒绝启动——所以要么五个都下，要么加载前在"模型配置"页把"MTP 草稿验证"或"图像输入"关掉。
（第 4 个换成 `mtp-…-shared-Q8_0.gguf` 也行，两者相差不到 1%。）
...
现在生成这个文件还需要一棵带编译好的工具链的源码树——安装包里没有 `llama-quantize` 和 numpy。做法在
[install.md 第 5 节](install.md#5-model-files)。`mtp-Qwen3.8-Flash-Next-shared-Q4_K_M-head-iq4_xs.gguf`
放进模型文件夹后点**重新扫描**，会被自动优先选用。可直接下载的版本计划在后续版本提供。

## 出问题了

- **模型页是空的。** 检查"模型目录"：只列出已登记目录下的 `.gguf`，分片模型按 `-00001-of-` 那个
  分片显示。点"重新扫描"。
- **点"加载模型"立刻失败，提示草稿或投影。** 第 4 或第 5 个文件不在模型文件夹里（或改过名）。补上并
  重新扫描，或者在配置页把对应开关关掉。

...
export default function StrixLlamaPage({ view }: { view: View }) {
  ...
  const visibleLogs = logText.split('\n').filter(l => l.toLowerCase().includes(logQuery.toLowerCase()) && (level === 'all' || (level === 'error' ? logLevel(l) === 'error' : logLevel(l) !== '')))

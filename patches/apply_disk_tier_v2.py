...
Switching between two long conversations on two slots cost ~5-6 s each time: --cache-idle-slots saves AND clears every idle slot on each new task with a unified KV, so the conversation left behind was written out (3.4 s for 5.7 GB) and the one returned to read back (2.6 s), on the main loop, although the cells were already allocated. Kept in their slots, switches are 0.3-0.4 s. They reach disk instead when the server is idle and one has grown by a block (4096 tokens): the main loop gathers the state (~0.1 s) and a background thread writes only what changed - checkpoints never change once made, and the KV grows by rows appended to every layer's section, so content-defined chunks of the state (XXH3-128 named) find the old bytes wherever they moved. Measured: a 79K-token conversation grown by 600 tokens wrote 0.52 GiB instead of 5.7, and restoring it after a TerminateProcess gave the same 16 tokens as the warm slot. Idle slots hand their checkpoints' bytes back to the store (a 79K conversation keeps 3.4 GB of them in system RAM otherwise; 5.86 -> 2.78 GB) and a rewind reads the one it needs back (38-49 ms), matching token for token a run that never paged. Kept conversations can fill the pool, so a completion makes room first - least recently used idle slots, written first - or a restore would fail and fall back to processing the whole prompt. Version 1 entries are still read and are converted in the background, streaming; a killed write leaves only .part files and unreferenced objects, which the next start removes.

Generated by tools/make_patch_script.py from the tree it must reproduce, so every anchor is
unique by construction. 18 hunks across 3 file(s).
...
''',
    ...
    size_t   disk_covered(const server_tokens & tokens);
    bool     disk_wants(const server_prompt & prompt) { return disk_covered(prompt.tokens) < prompt.tokens.size(); }
...
struct spc_out {
    ...
    bool close() {
        ...
        if (used > 0 && !bad) {
            const size_t padded = (used + 4095) & ~(size_t) 4095;
...
// the header of an entry: its tokens, and for version 2 the objects it names
bool scan_entry(spc_stage & st, disk_entry & e, bool has_mtmd, std::string & why) {
    spc_file f(st);
    if (!f.open(e.path)) {
        why = "cannot open it";
        return false;
...
bool read_ckpt(spc_file & f, const std::string & path, const disk_ckpt_ref & r, common_prompt_checkpoint & c) {
    ...
    const bool ok = f.read(magic, 8) && memcmp(magic, CKP_MAGIC, 8) == 0 && get(f, version) && version == CKP_V1
        && get(f, n_tokens) && get(f, pmin) && get(f, pmax)
        && n_tokens == r.n_tokens && pmin == r.pos_min && pmax == r.pos_max
        && get_blob(f, c.data_tgt, 1ull << 40) && get_blob(f, c.data_dft, 1ull << 40) && get_blob(f, c.data_spec, 1ull << 40)
        && c.data_tgt.size() == r.size_tgt && c.data_dft.size() == r.size_dft && c.data_spec.size() == r.size_spec;
    f.close();
    if (ok) {
        c.n_tokens = n_tokens;
        c.pos_min  = pmin;
...
// a whole entry into memory. Version 2 checks every chunk against its name: a state that is wrong in one
// byte decodes into nonsense, and XXH3 over 2.4 GiB costs ~0.1 s
bool read_entry(spc_stage & st, const std::string & dir, const disk_entry & e, bool has_mtmd,
                server_prompt_cache_state & state, std::string & why) {
    spc_file f(st);
    if (e.version == (int32_t) SPC_V1) {
        ...
        bool ok = f.open(e.path) && f.read(magic, 8) && memcmp(magic, SPC_MAGIC, 8) == 0 && get(f, version)
               ...
               && get_blob(f, state.data.main, 1ull << 40) && get_blob(f, state.data.drft, 1ull << 40);
        uint32_t n_ckpt = 0;
    ...
    auto fill = [&](std::vector<uint8_t> & v, uint64_t size, const std::vector<disk_chunk_ref> & refs) -> bool {
        ...
        for (const auto & c : refs) {
            ...
            if (!ok) {
                why = "missing chunk " + chunk_hex(c.hi, c.lo);
                return false;
            }
            const disk_chunk_ref got = chunk_ref(v.data() + off, c.size);
            if (got.hi != c.hi || got.lo != c.lo) {
                why = "chunk " + chunk_hex(c.hi, c.lo) + " does not match its name";
                return false;
            }
            off += c.size;
        }
...
void server_prompt_cache::set_disk(const std::string & dir, size_t limit_mib, bool has_mtmd) {
    std::error_code ec;
    fs::create_directories(fs::path(dir) / "ckpt", ec);
    fs::create_directories(fs::path(dir) / "chunks", ec);
    disk_dir      = dir;
    ...
    // 2. entries, and a reference for each object they name
    spc_stage stage;
    int n_v1 = 0, n_v2 = 0;
    for (const auto & de : fs::directory_iterator(dir, ec)) {
        if (!de.is_regular_file() || de.path().extension() != ".spc") {
            continue;
        }
        disk_entry e;
        ...
        e.order = mtime_ms(de);
        std::string why;
        if (!scan_entry(stage, e, disk_has_mtmd, why)) {
            // unreadable here is unreadable for good: no lookup can ever restore it
            SRV_WRN(" - disk cache: dropping %s (%.2f GiB): %s\\n", de.path().filename().string().c_str(),
                    e.bytes / (1024.0 * 1024.0 * 1024.0), why.c_str());
            fs::remove(de.path(), ec);
            continue;
        }
        (e.version == (int32_t) SPC_V1 ? n_v1 : n_v2)++;
        disk_seq = std::max(disk_seq, e.order);
    ...
    // 3. objects: size what is referenced, remove what is not
    size_t n_orphans = 0;
    std::vector<fs::path> orphans;
    for (auto it = fs::recursive_directory_iterator(fs::path(dir) / "chunks", ec); !ec && it != fs::recursive_directory_iterator(); it.increment(ec)) {
        if (!it->is_regular_file(ec) || it->path().extension() != ".chk") {
            continue;
        }
        ...
        if (o == disk_chunks.end()) {
            ...
            o->second.bytes = (uint64_t) it->file_size(ec);
        }
    }
    for (const auto & de : fs::directory_iterator(fs::path(dir) / "ckpt", ec)) {
        if (!de.is_regular_file() || de.path().extension() != ".ckp") {
            continue;
        }
        char * end = nullptr;
    ...
    // 4. an entry whose objects are gone cannot be restored
    for (auto it = disk_index.begin(); it != disk_index.end();) {
        bool whole = true;
        for (const auto * refs : { &it->main, &it->drft }) {
            for (const auto & c : *refs) {
                whole = whole && disk_chunks[{ c.hi, c.lo }].bytes > 0;
            }
        }
        ...
        SRV_WRN(" - disk cache: dropping %s: objects it names are missing\\n", fs::path(it->path).filename().string().c_str());
        disk_drop(*it);
        it = disk_index.erase(it);
    }
    disk_bytes = 0;
    ...
    SRV_INF("disk prompt cache: %zu entries (%d to convert), %.1f GiB in %s (limit %.1f GiB, %zu chunks, %zu checkpoints, "
            "write every %d tokens of growth%s)\\n",
            disk_index.size(), n_v1, disk_bytes / (1024.0 * 1024.0 * 1024.0), dir.c_str(), disk_limit / (1024.0 * 1024.0 * 1024.0),
            disk_chunks.size(), disk_ckpts.size(), disk_block, n_orphans ? (", " + std::to_string(n_orphans) + " orphans removed").c_str() : "");
    GGML_UNUSED(n_v2);
}
...
// does the entry's own file. Only the writer thread calls this (and set_disk, before it starts).
void server_prompt_cache::disk_drop(const disk_entry & e) {
    std::error_code ec;
    for (const auto * refs : { &e.main, &e.drft }) {
        for (const auto & c : *refs) {
...
size_t server_prompt_cache::disk_covered(const server_tokens & tokens) {
    if (disk_limit == 0) {
        return 0;
    }
    std::lock_guard<std::mutex> lk(disk_mu);
    return disk_covered_locked(tokens, true);
}

// caller holds disk_mu: a job that is queued or being written and shares enough of a prompt to be what a
// load of it would want
bool server_prompt_cache::disk_in_flight(const server_tokens & tokens) {
    if (!disk_thread.joinable() || disk_stop) {
        return false;
    }
    auto relevant = [&](const disk_job & j) {
        const size_t lcp = j.tokens.get_common_prefix(tokens);
        return lcp > 0 && lcp * 4 >= j.tokens.size();
    };
    for (const auto & j : disk_queue) {
        if (relevant(*j)) {
            return true;
...
bool server_prompt_cache::persist(const server_prompt & prompt, std::vector<uint8_t> && data_main, std::vector<uint8_t> && data_drft, bool wait,
    ...
    // that was saved before carries all of them but its newest. A handed-back one the store has since
    // evicted cannot be written, so the entry goes without it rather than naming something missing.
    std::vector<bool> need(refs.size(), false);
    {
        std::lock_guard<std::mutex> lk(disk_mu);
        for (size_t i = 0; i < refs.size(); ++i) {
            const bool stored = disk_ckpts.count(refs[i].first.key) || disk_ckpt_in_flight(refs[i].first.key);
            if (refs[i].second) {
                need[i] = !stored;
...
uint64_t server_prompt_cache::ckpt_page_out(const server_tokens & tokens, std::list<common_prompt_checkpoint> & ckpts, ckpt_paged_map & paged) {
    ...
    for (auto & c : ckpts) {
        ...
        paged[{ c.n_tokens, c.pos_min, c.pos_max }] = r;
        freed += c.size();
...
bool server_prompt_cache::ckpt_page_in(const server_tokens & tokens, const ckpt_paged_map & paged, common_prompt_checkpoint & c) {
    ...
    if (!read_ckpt(f, ckpt_path(disk_dir, r.key), r, tmp)) {
        return false;
    }
...
void server_prompt_cache::disk_writer() {
    ...
    while (true) {
        ...
        disk_cv.wait(lk, [&] { return disk_stop || !disk_queue.empty() || !disk_migrate.empty() || !disk_doomed.empty(); });
    }
}
...
void server_prompt_cache::disk_write(disk_job & job) {
    ...
    auto store = [&](const std::vector<uint8_t> & v, std::vector<disk_chunk_ref> & refs) {
        for (size_t off = 0; ok && off < v.size();) {
            ...
            if (!have && fresh.insert({ c.hi, c.lo }).second) {
                const std::string path = chunk_path(disk_dir, c.hi, c.lo);
                std::error_code ec;
                fs::create_directories(fs::path(path).parent_path(), ec);
                uint64_t b = 0;
                ok = write_part(st, path + ".part", [&](spc_out & o) { return o.write(v.data() + off, n); }, b)
                  && place(path + ".part", path);
                wrote += n;
            }
            off += n;
        }
    ...
    for (const auto & [key, c] : job.ckpt_data) {
        ...
        const std::string path = ckpt_path(disk_dir, key);
        uint64_t b = 0;
        ok = write_part(st, path + ".part", [&](spc_out & o) {
                return o.write(CKP_MAGIC, 8) && put(o, CKP_V1) && put(o, (int64_t) c.n_tokens)
                    && put(o, (int32_t) c.pos_min) && put(o, (int32_t) c.pos_max)
                    && put_blob(o, c.data_tgt) && put_blob(o, c.data_dft) && put_blob(o, c.data_spec);
            }, b) && place(path + ".part", path);
        if (ok) {
            fresh_ckpt[key] = b;
            wrote_ckpt += b;
        }
    ...
    std::lock_guard<std::mutex> lk(disk_mu);
    // every checkpoint the manifest names has to exist: one that was in flight elsewhere when this job
    // was made, in a job that then failed, is missing and makes the entry useless
    for (const auto & k : e.ckpts) {
        if (!disk_ckpts.count(k.key) && !fresh_ckpt.count(k.key)) {
            SRV_WRN(" - disk cache: checkpoint %016llx went missing, not recording %d tokens\\n", (unsigned long long) k.key, (int) e.tokens.size());
            std::error_code ec;
    ...
    // an entry of the same tokens (a race this job lost) gives way to this one
    for (auto it = disk_index.begin(); it != disk_index.end(); ++it) {
        if (it->path == path) {
            disk_drop(*it);
            disk_index.erase(it);
...
// however large the entry, because the chunker only ever needs CDC_MAX bytes ahead of its cut
void server_prompt_cache::disk_convert_v1(const std::string & path) {
    ...
    uint32_t version = 0, mtmd = 0;
    uint64_t ntb = 0;
    bool ok = f.open(path) && f.read(magic, 8) && memcmp(magic, SPC_MAGIC, 8) == 0 && get(f, version)
    ...
    for (uint32_t i = 0; ok && i < n_ckpt; ++i) {
        ...
        ok = get(f, n_tokens) && get(f, pos_min) && get(f, pos_max);
        // the key needs all three sizes, so the file is named once they are known
        uint64_t b = 0;
        ok = ok && write_part(out_st, tmp, [&](spc_out & o) {
                bool w = o.write(CKP_MAGIC, 8) && put(o, CKP_V1) && put(o, n_tokens) && put(o, pos_min) && put(o, pos_max);
                for (int k = 0; w && k < 3; ++k) {
                    uint64_t n = 0;
                    w = get(f, n) && put(o, n);
                    sizes[k] = n;
    ...
    e.path  = mpath;
    e.bytes = mbytes;
    e.order = order;                               // same place in the LRU order as the file it replaces
    disk_bytes += mbytes;
...
server_prompt_cache_state * server_prompt_cache::load_from_disk(const server_tokens & tokens_new, float & f_keep_best, float & f_sim_best) {
    ...
    disk_cv.wait(lk, [&] { return !disk_in_flight(tokens_new); });
...
''',
    ...
    std::string why;
    spc_stage stage;
    if (!read_entry(stage, disk_dir, *best, disk_has_mtmd, state, why)) {
...
''',
    ...
    // strixllama: this slot's checkpoints whose bytes went back to the disk tier (see persist_idle_slots)
...
''',
         '''        if (ret) {
            // strixllama: room for this prompt and a margin to generate into, before anything is restored
            if (task.type == SERVER_TASK_TYPE_COMPLETION) {
                make_room(*ret, task.tokens.size() + 1024);
            }
...
''',
    ...
    void persist_idle_slots() {
        ...
        for (auto & slot : slots) {
            ...
            const size_t covered = prompt_cache->disk_covered(slot.prompt.tokens);
            if ((size_t) n < covered + (size_t) prompt_cache->disk_block) {
                slot.n_tokens_persist_checked = n;          // not a block's worth yet
                continue;
            }
            if (slot.prompt_save(*prompt_cache, /* ram = */ false, /* wait = */ false)) {
                slot.n_tokens_persist_checked = n;
            }                                               // else the writer was busy: next time
        }
...
''',
         '''                                    bool do_reset = it == slot.prompt.checkpoints.rend();
                                    ...
                                    // a rewind picks is read back now - and if it cannot be, the prompt is processed
                                    if (!do_reset && it->data_tgt.empty()) {
                                        const int64_t t_in = ggml_time_us();
                                        do_reset = !(prompt_cache && prompt_cache->ckpt_page_in(slot.prompt.tokens, slot.ckpt_paged, *it));
                                        if (do_reset) {
                                            SLT_WRN(slot, "%s", "a checkpoint handed back to the disk tier could not be read back, processing the prompt\\n");
                                        } else {
